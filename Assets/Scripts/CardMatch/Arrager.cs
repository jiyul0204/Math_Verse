...
public class Arrager : MonoBehaviour
{
    ...
    public int GetIndexByPosition(Transform card, int skipIndex=-1)
    {
        ...
        {
            if (card.position.x < children[i].position.x) break;
            else if (skipIndex != i) result++;
        }
    ...
    {
        GetDragMessage.SwapCards(children[index01], children[index02]);
        UpdateChildren();
    }
